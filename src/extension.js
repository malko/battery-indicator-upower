...
const upowerParser = output => {
  const items = String(output)
    .split('\n\n')
    .filter(_ => {
      return (
        _.match(/^Device:/) && // only devices
        _.match(/^\s*serial:/m)
      ); // remove if no serial
    })
    .map(deviceStr => {
      const lines = deviceStr.split('\n').map(line => {
        ...
        if (!groups) return { prop: 'type', val: line.trim() };
        let { prop, val } = groups;
...
const getDeviceIcon = (device, useSymbolic) => {
  ...
  if (model.match(/keyboard/i)) return deviceTypeIcons.keyboard + suffix;
  if (model.match(/mouse/i)) return deviceTypeIcons.mouse + suffix;
  if (model.match(/Sony PLAYSTATION\(R\)3 Controller Motion Sensors/i))
    return deviceTypeIcons['gaming-input'] + suffix;
  if (type in deviceTypeIcons) return deviceTypeIcons[type] + suffix;
...
const Indicator = GObject.registerClass(
  class Indicator extends PanelMenu.Button {
    _init() {
      ...
      this._indicatorsBoxLayout = new St.BoxLayout({
        style_class: 'battery-indicator-boxlayout',
      });

      // add a loading icon while initializing
      this.add_child(this._indicatorsBoxLayout);
      this._indicatorsBoxLayout.add_child(
        new St.Icon({
    ...
    async refreshIndicator() {
      ...
      const hiddenDevices = settingsManager
        .get('hidden-devices')
        .filter(serial =>
          devices.find(d => d?.serial === serial) ? true : false
        );
      ...
      devices.length &&
        devices.forEach((device, id) => {
          ...
          log(
            `device ${device.model} got icon: ${icon_name} (useSymbolic: ${useSymbolic})`
          );
          if (displayed) {
